...
class BaseRequestSchema(BaseModel):
    ...
    requested_amount: float = PydanticField(
        ..., json_schema_extra={"example": 1000.0}, ge=0, description="Monto solicitado."
    )
    term_months: int = PydanticField(
        ..., json_schema_extra={"example": 12}, gt=0, description="Plazo del crédito en meses."
    )
    annual_interest_rate: float = PydanticField(
        ..., json_schema_extra={"example": 0.1}, ge=0, description="Tasa de interés anual."
    )
    ...
    status_id: UUID = PydanticField(
        ...,
        json_schema_extra={"example": "c1d2e3f4-a5b6-7890-1234-567890abcdef"},
        description="ID del estado de la solicitud.",
    )
    approved_amount: Optional[float] = PydanticField(
        None, json_schema_extra={"example": 950.0}, ge=0, description="Monto final aprobado, si aplica."
    )
    approved_at: Optional[datetime] = PydanticField(
        None,
        json_schema_extra={"example": "2025-05-29T12:00:00Z"},
    ...
    rejection_reason: Optional[str] = PydanticField(
        None,
        json_schema_extra={"example": "Falta de historial crediticio."},
        max_length=500,
        description="Razón si fue rechazada.",
    )
    risk_score: Optional[float] = PydanticField(
        None,
        json_schema_extra={"example": 73},
        ge=0,
        le=1000,
    ...
    number_of_dependents: Optional[int] = PydanticField(
        None, json_schema_extra={"example": 2}, ge=0, description="Número de dependientes del solicitante."
    )
    other_income_sources: Optional[float] = PydanticField(
        None, json_schema_extra={"example": 100.0}, ge=0, description="Otros ingresos del solicitante."
    )
    ...
    created_at: Optional[datetime] = PydanticField(
        None,
        json_schema_extra={"example": "2025-05-29T10:00:00Z"},
        description="Fecha de creación de la solicitud.",
    )
    updated_at: Optional[datetime] = PydanticField(
        None,
        json_schema_extra={"example": "2025-05-29T10:05:00Z"},
